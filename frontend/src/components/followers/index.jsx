...
export default function Follower () {
  ...
  useEffect(() => {
    ...
    let url = 'http://127.0.0.1:5000/follow/' + id + '/followers' + '?page=' + 1 + '&pagesize=' + 50
    fetch(url, {
      method: 'GET',
      headers: {
      ...
      .then(data => {
        if (data.message) {
          if (data.message.follow) {
            if (data.message.follow.includes('you have no fans yet')) {
          ...
          console.log('your follow list', data.follows)
        }
      });
  }, [])
  ...
  return (
    <List
        header={<div style = {{ fontsize: '80px' }}>Followers</div>}
